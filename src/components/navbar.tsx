...
import { Search, Bell, User, Settings, CreditCard, LogOut, Clapperboard, Sparkles, Menu } from "lucide-react"
        ...
        <div className="container mx-auto px-4 h-16 flex items-center justify-between gap-4">
          ...
          <div className={`flex-1 max-w-md relative ${isSearchOpen ? 'block absolute inset-x-0 mx-4 bg-background z-50' : 'hidden sm:block'}`}>
            ...
            <Input 
              ...
              className="pl-10 bg-muted/50 border-none focus-visible:ring-1 focus-visible:ring-accent h-10"
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" size="icon" className="relative group">
                  ...
                  <span className="absolute top-2 right-2 w-2 h-2 bg-accent rounded-full border border-background animate-pulse" />
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end" className="w-80 bg-card border-border shadow-xl p-0 overflow-hidden">
                ...
                <div className="p-2 border-t text-center">
                  <Button variant="ghost" size="sm" className="w-full text-accent text-xs hover:bg-accent/10">View all notifications</Button>
                </div>
              </DropdownMenuContent>
            </DropdownMenu>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <div className="w-9 h-9 rounded-full bg-gradient-to-tr from-primary/20 to-accent/20 flex items-center justify-center border border-accent/20 cursor-pointer hover:border-accent/50 hover:bg-primary/10 transition-all overflow-hidden group">
                  <User className="w-5 h-5 text-accent group-hover:scale-110 transition-transform" />
                </div>
              </DropdownMenuTrigger>
